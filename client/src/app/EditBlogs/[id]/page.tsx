...
const page = () => {
  const { id } = useParams(); // this is your blogId
  const [userDetail, setUser] = useState<any>(null);
  ...
  // Fetch user details using the userId from blog
  ...
  const getBlogById = async () => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/blog/getBlog/${id}`,
        {
          method: "GET",
        }
      );
      ...
      if (response.ok) {
        ...
        // Once blog is fetched, get the user details using the userId from the blog
        if (data.message?.userId) {
          getUserDetail(data.message.userId);
        }
        setBlogImage(data.message.blog_image);
        form.setValue("text", data.message.Content);
        form.setValue("type", data.message.Blog_Type);
      } else {
        console.log(data.message);
      }
    } catch (error: any) {
